	...
	public class LandingZoneLogic
	{
		...
		private void DropPiecesToRestingPosition(List<SimPiece> pieces)
		{
			foreach (SimPiece piece in pieces)
			{
				...
				if (!TryPlacePiece(piece.PieceId, piece.ShapeId, landingRow, piece.SpringboardColumn, out placePieceStatus))
				{
		...
		public void StartNewPiecesPositioning(List<SimPiece> sprungPieces)
		{
			...
			{
				if (!TryPlacePiece(piece.PieceId, piece.ShapeId, NumRowsInLandingZone - 1 - 3, piece.SpringboardColumn, out placePieceStatus))
		...
		public bool TryPlacePiece(int pieceId, int shapeId, int row, int col, out PlacePieceStatus placePieceStatus)
		{
			...
			try
			{
				piece = SimPieceLibrary.SimPieces[shapeId];
			}
			catch (Exception)
			{
